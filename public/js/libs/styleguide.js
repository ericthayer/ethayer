...
var StyleguideIndex = {
  init: function() {
    if ($('.sg-iframe-content').length && !FRONT_END_TEST) return false;

    this.$body = $('body');
    this.$breakpointsLinks = $('.sg-header-breakpoints-item-link');
    this.$sidebarContent = $('.sg-sidebar-content');
    this.$sidebarLinks = $('.sg-sidebar-nav-item-link');
    this.$sidebarToggle = $('[class*="sg-sidebar-toggle"]');
    this.$iframe = $('.sg-iframe-wrapper iframe');
    this.$iframeContent = this.isChromeAndFileProtocol() ? null : $('.sg-iframe-wrapper iframe').contents();
